...
class Editor:
    ...
    def read_filename(self, file_name):
        ...
        #Feel free to implement your own way [We can decide which implementation to use later]
        file = open(file_name, "r")
        fileLines = file.readlines()
        for i in range(len(fileLines)):
            self.text_lines.insert(i, fileLines[i])
    ...
    def delete_num(self, line_num):
        ...
        if line_num == 0:
            raise EditorError('?')
        elif not self.text_lines.is_empty:
